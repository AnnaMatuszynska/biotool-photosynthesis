...
def vXcyc(
    V: float, H: float, nHX: float, KphSatZ: float, kDeepoxV: float, kEpoxZ: float, Xtot: float
) -> float:
    """Xanthophyll cycle"""
...
def vPsbSP(
    Pr: float, H: float, nHL: float, KphSatLHC: float, kProt: float, kDeprot: float, PsbStot: float
) -> float:
