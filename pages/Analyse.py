...
with col1:
    slider_time = st.slider(
        _('SLIDER_TIME'),
        5, 30,  # Zwischenschritte können durch folgendes angegeben werden: (x,y,z)
    )
with col2:
...
if version == 'expert':
    ...
    with col1:
        slider_aktivation = st.slider(
            _('SLIDER_ACTIVATION'),
            -1000, +1000, 0,  # Zwischenschritte können durch folgendes angegeben werden: (x,y,z)
        )
    with col2:
...
if version == 'simple':
    with st.expander(_('KONTROLL')):
        st.markdown(
            _('INTRODUCTION_QUESTION'))
        option1 = st.radio(
            _('QUESTION_1'),
            ('None', _('AW1'), _('AW2')), index=0)
        if option1 == _('AW1'):
            st.markdown(
                _('RA1'))
        elif option1 == _('AW2'):
