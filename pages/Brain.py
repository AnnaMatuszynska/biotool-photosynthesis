...
from model import Simulator, M16model
import numpy as np
...
language = st.sidebar.selectbox('⚙ Language 🌍💬', ['German', 'English'], label_visibility="visible")
try:
    localizator = gettext.translation('b-brain', localedir=os.path.join('locales', version), languages=[language])
    localizator.install()
...
with col1:
    slider_light = st.slider(
        ...
        100, 900,  # Zwischenschritte können durch folgendes angegeben werden: (x,y,z)
    )
with col2:
...
if version == 'expert':
    ...
    with col1:
        slider_aktivation = st.slider(
            _('SLIDER_ACTIVATION'),
            -100, +100, 0,  # Zwischenschritte können durch folgendes angegeben werden: (x,y,z)
        )
    with col2:
...
tprot = np.array([1., 0.8, 28, 0.8, 30, 0.8,
                  50, 0.8, 69, 0.8, 90, 0.8,
                  110, 0.8, 130, 0.8, 151, 0.8,
                  171, 0.8, 29, 0.8, 49, 0.8,
                  70, 0.8, 90, 0.8, 109, 0.8,
                  slider_time * 60, 0.8, 30, 0.8, 30, 0.8,
                  50, 0.8, 70, 0.8, 90, 0.8,
                  110, 0.8])

ProtPFDs = np.array([0., 5000., 0., 5000., slider_light, 5000.,
                     slider_light, 5000., slider_light, 5000., slider_light, 5000.,
                     slider_light, 5000., slider_light, 5000., slider_light, 5000.,
                     slider_light, 5000., 0., 5000., 0., 5000.,
                     0., 5000., 0., 5000., 0., 5000.,
                     0., 5000., 0., 5000., slider_light, 5000.,
                     slider_light, 5000., slider_light, 5000., slider_light, 5000.,
                     slider_light, 5000.])
...
if st.button('Start', type="primary"):
    with st.spinner(_('SPINNER')):  # loading indicator
        ...
        Fm, NPQ, tm, Fo, to, PhiPSII = get_NPQ(PAM.get_variable('Fluo'), PAM.get_time(), PAM.get_variable('L'),
                                               maxlight=5000)

        # maingraph
        chart_data = pd.DataFrame({
            'Fluoreszenz': F / max(F),
            'Zeit': PAM.get_time() / 60
        })

        chart = alt.Chart(chart_data).mark_line(color='#FF4B4B').encode(
            x='Zeit',
            y='Fluoreszenz',
            tooltip="Fluoreszenz"
        )
        ...
        areas = alt.Chart(
            areas_data.reset_index()
        ).mark_rect(
            opacity=0.3  # Durchsichtigkeits
        ).encode(
            x2='stop',
            y=alt.value(0),
        ...
        text = alt.Chart(areas_data).mark_text(
            align='left',
            baseline='middle',
            dx=7,
            dy=-135,
        ...
        if version == 'expert':
            ...
            with left:
                ...
                points1 = alt.Chart(chart_data1).mark_point(filled=True, size=65, color='#FF4B4B').encode(
                    x=alt.X('Zeit'),
                    tooltip="NPQ",
                    y=alt.Y('NPQ', axis=alt.Axis(title='NPQ [a.u.]'))
                )
            ...
            with right:
                ...
                points2 = alt.Chart(chart_data2).mark_point(filled=True, size=65, color='#FF4B4B').encode(
                    x=alt.X('Zeit'),
                    tooltip="Phi",
                    y=alt.Y('Phi', axis=alt.Axis(title='Φ(PSII)'))
                )
